...
namespace App\Services\Image\Thumb\Drivers; 
...
class Gd extends ImageDriver
{
    ...
    public function crop($properties)
    {
        ...
        // Get the current width and height
        $srcWidth = imagesx($this->tmp_image);
        $srcHeight = imagesy($this->tmp_image);
		...
		//$img = imagecreatetruecolor($srcWidth/$k-$xDiff, $srcHeight/$k-$yDiff); 
		if ($status = imagecopyresampled($img, $this->tmp_image, 0, 0, $xDiff/2*$k, $yDiff/2*$k, $srcWidth/$k-$xDiff, $srcHeight/$k-$yDiff, $srcWidth-$xDiff*$k, $srcHeight-$yDiff*$k)) {
            // Swap the new image for the old one
            imagedestroy($this->tmp_image);
            $this->tmp_image = $img;		
		}
