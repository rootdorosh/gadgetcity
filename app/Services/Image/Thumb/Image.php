...
namespace App\Services\Image\Thumb; 

use Yii;
use App\Services\Image\Thumb\Drivers\ImageDriver;
use App\Services\Image\Thumb\Drivers\Gd;
...
class Image
{
    ...
    public function __construct($image, $config = NULL)
    {
        ...
        $this->driver = new \App\Services\Image\Thumb\Drivers\Gd($this->config['params']);

        // Validate the driver
        if (!($this->driver instanceof ImageDriver))
            die('image driver must be implement ImageDriver class');
    }
    ...
    {
        if (!$this->valid_size('width', $width))
            die('image invalid width: ' . $width);

        if (!$this->valid_size('height', $height))
            die('image invalid height: ' . $height);

        if (!$this->valid_size('top', $top))
            die('image invalid top: ' . $top);

        if (!$this->valid_size('left', $left))
            die('image invalid left: ' . $left);
    ...
    public function save($new_image = false, $chmod = 0644, $keep_actions = false)
    {
        ...
        if (!is_writable($dir))
            die('image directory unwritable');
